 ...
 * 1. /theme/lib/styles/css/main.css
 *
 * Enqueue scripts in the following order:
 * 1. jquery-1.11.1.min.js via Google CDN
 * 2. /theme/lib/javascripts/vendor/modernizr.min.js
 * 3. /theme/lib/javascripts/global.js (in footer)
 *
...
function roots_scripts() {
  ...
  if (WP_ENV === 'development') {  
    $assets = array( 
      ...
      'picfill'   => '/lib/javascripts/picturefill-min.js',
    );
  } else {
    $get_assets = file_get_contents(get_template_directory() . '/lib/manifest.json');
  ...
  wp_enqueue_script('picfill', get_template_directory_uri() . $assets['picfill'], array(), null, true);
  wp_enqueue_script('roots_js', get_template_directory_uri() . $assets['js'], array(), null, true);
  wp_localize_script('roots_js', 'cb_ajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ));
...
function roots_jquery_local_fallback($src, $handle = null) {
  ...
  if ($add_jquery_fallback) {
    echo '<script>window.jQuery || document.write(\'<script src="' . get_template_directory_uri() . '/lib/javascripts/jquery-fallback.js"><\/script>\')</script>' . "\n";
...
<?php } 
add_action('wp_footer', 'woo_pip_client_js', 21);
