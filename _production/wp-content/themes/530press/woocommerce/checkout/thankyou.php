...
if ( $order ) : ?>
	...
	<?php else : ?>
		...
		<ul class="order_details order section-region">
			...
			<li class="date order__item">
				<small><?php _e( 'Date:', 'woocommerce' ); ?></small>
				<div><strong><?php echo date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) ); ?></strong></div>
			</li>
			<li class="total order__item">
				<small><?php _e( 'Total:', 'woocommerce' ); ?></small>
				<div><strong><?php echo $order->get_formatted_order_total(); ?></strong></div>
			</li>
			<?php if ( $order->payment_method_title ) : ?>
			<li class="method order__item">
				<small><?php _e( 'Payment method:', 'woocommerce' ); ?></small>
				<div><strong><?php echo $order->payment_method_title; ?></strong></div>
			</li>
			<?php endif; ?>
		</ul>
